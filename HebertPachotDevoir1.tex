...
En observant le graphique temporel (cf. fig.~\ref{gretzkyPlot}), on se rend compte que c’est la première moitié de la carrière qui est exceptionnelle. Il y aurait même une corrélation négative entre le nombre de buts par saison et les saisons. En effet, si l’on ajoute manuellement (tableur\dots) une deuxième colonne représentant les saisons, on obtient une corrélation négative de -0,85 entre les deux variables. Moralité : plus on gagne de l’expérience, moins on est bon!
...
> gretzky2<-read.table(file="~/d1_gretzky.csv")
> cor(gretzky2$V1, gretzky2$V2)
[1] -0.8507047
\end{verbatim}
